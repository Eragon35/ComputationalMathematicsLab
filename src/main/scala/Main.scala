...
object Main {
  ...
  def initializer(n: Int): Unit = {
    ...
    if (matrix.filter(a => a.size != matrix.size + 1).toList.nonEmpty) throw new Exception("Матрица не квадратичная") // checking if matrix is square
    ...
    if (determinant == 0) throw new Exception("Определитель = 0") // checking if matrix has roots
    else {
      findTriangleMatrix()
      findSolution()
      findResidual()
